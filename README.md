...
- **Live controls**: drag queue cards to reorder, drop one on a deck to crossfade it in now, talkover button dips the music for toasts, keyboard shortcuts (Space, ←/→, Shift+←/→ seek, ↑/↓, T, C), click/drag the waveform to seek, and wall-clock ETAs on every queued track.
- **FX pad**: horn / riser / drop / laser / brake one-shots with cooldowns, synthesized through a shared reverb+echo space. Prefer real recordings? Drop mp3s into [public/fx/](public/fx/README.txt) and they override the synths automatically.
